...
// Mirrors TizenBrew's createAdbConnection exactly
function createAdbConnection(ip) {
  return new Promise(function (resolve, reject) {
    ...
    // TizenBrew uses 1000ms waitTimeout then resolves if connected (desktop path)
    // On TV it resolves immediately on connect — we're on TV so do the same
...
function getDebugPort(appId) {
  ...
  return createAdbConnection().then(function (client) {
    return new Promise(function (resolve, reject) {
      ...
      // Mirrors getDuid from Buxton2.js — createStream then listen for data
